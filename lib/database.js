...
const database = () => {
    ...
    self.add = _itemd => {
        ...
        const names = _.d.list(itemd, "name", [])
        if (_.is.Empty(names)) {
            console.log("warning: no name - skipping");
            return;
        }

        _db_item[id] = itemd;

        names.forEach(name => _add_list(_db_names, name.toLowerCase(), id));
    ...
    self.all = () => _.values(_db_item);

    // more "global"
    self.item_theme_parts = (itemd) => {
        const themes = _.d.list(itemd, "_theme", [])
        if (_.is.Empty(themes)) {
            return;
        }

        return _.flatten(themes.map(_theme_parts), true);
    }
